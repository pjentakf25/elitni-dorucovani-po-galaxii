...
# =========================================================================
# STATIONS & ECONOMIC DATA (Spatially adjusted for mega sizes)
# =========================================================================
stations = [
    {
        "name": "Death Star Sector 01",   
        "x": 4500, "y": -5000, "variant": 0,  
        "prices": {"Metals": 55, "Food": 25, "Tech": 130} 
    },
    {
        "name": "Sol Megastructure Echo", 
        "x": -6000, "y": 4500, "variant": 1,  
        "prices": {"Metals": 32, "Food": 35, "Tech": 90}
    },
    {
        "name": "Zeta Reticuli Outpost", 
        "x": 7500, "y": 7000, "variant": 2,   
        "prices": {"Metals": 48, "Food": 60, "Tech": 55}  
    },
    {
        "name": "Orion Industrial Hub",  
        "x": -4500, "y": -9500, "variant": 3,  
        "prices": {"Metals": 12, "Food": 40, "Tech": 145}  
    },
    {
        "name": "Imperial Core Station",  
        "x": 9500, "y": -4000, "variant": 0,  
        "prices": {"Metals": 60, "Food": 20, "Tech": 120}
    },
    {
        "name": "Kepler Dyson Depot",     
        "x": -8000, "y": -6500, "variant": 1, 
        "prices": {"Metals": 22, "Food": 50, "Tech": 110}
    },
]
...
radio_messages_general = [
    ...
    "RCS: Micro-thruster calibration sequence complete.",
...
def draw_station(surface, cx, cy, variant, rot, bt):
    ...
    if variant == 0:
        ...
        # MASSIVE Zeta Hexagonal Outpost Layout
        points = []
        for i in range(6):
        ...
        # Internal superstructure frameworks
...
def draw_station_gate(surface, sx, sy, bt):
    # Visible giant landing deck corridor gate indicator
    gw, gh = 120, 24
    gx = sx - gw//2
    gy = sy + 120 
...
def add_rcs_particle(x, y, vx, vy):
    rcs_particles.append([x, y, vx, vy, random.uniform(0.10, 0.25)])
...
def update_and_draw_particles(surface, dt):
    ...
    for rp in rcs_particles[:]:
        rp[0] += rp[2]*dt; rp[1] += rp[3]*dt; rp[4] -= dt
        if rp[4] <= 0: rcs_particles.remove(rp); continue
        pygame.draw.circle(surface, LGRAY, (int(rp[0]), int(rp[1])), 1)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if game_state == 'MAIN_MENU':
                ...
                elif pygame.Rect(280, 300, 320, 40).collidepoint(mouse_pos): game_state = 'SHOP_MENU'
                elif pygame.Rect(280, 350, 320, 40).collidepoint(mouse_pos): running = False
            
            elif game_state == 'SHOP_MENU':
                if pygame.Rect(100, 230, 680, 35).collidepoint(mouse_pos) and credits_val >= 80:
                    credits_val -= 80; max_fuel += 50.0; fuel = max_fuel
                elif pygame.Rect(100, 280, 680, 35).collidepoint(mouse_pos) and credits_val >= 100:
                    credits_val -= 100; max_hull += 25.0; hull = max_hull
                elif pygame.Rect(100, 330, 680, 35).collidepoint(mouse_pos) and credits_val >= 120 and rcs_level < 3:
                    credits_val -= 120; rcs_level += 1
                ...
                if pygame.Rect(650, 150, 110, 28).collidepoint(mouse_pos) and credits_val >= 20 and fuel < max_fuel:
                    credits_val -= 20; fuel = max_fuel
                elif pygame.Rect(650, 182, 110, 28).collidepoint(mouse_pos) and credits_val >= 30 and hull < max_hull:
                    credits_val -= 30; hull = max_hull
                elif pygame.Rect(450, 326, 80, 26).collidepoint(mouse_pos):  
                    if total_loaded_cargo < max_cargo and credits_val >= p_map["Metals"]:
                        credits_val -= p_map["Metals"]; inventory["Metals"] += 1
                elif pygame.Rect(540, 326, 80, 26).collidepoint(mouse_pos):  
                    if inventory["Metals"] > 0:
                        credits_val += p_map["Metals"]; inventory["Metals"] -= 1
                elif pygame.Rect(450, 366, 80, 26).collidepoint(mouse_pos):  
                    if total_loaded_cargo < max_cargo and credits_val >= p_map["Food"]:
                        credits_val -= p_map["Food"]; inventory["Food"] += 1
                elif pygame.Rect(540, 366, 80, 26).collidepoint(mouse_pos):  
                    if inventory["Food"] > 0:
                        credits_val += p_map["Food"]; inventory["Food"] -= 1
                elif pygame.Rect(450, 406, 80, 26).collidepoint(mouse_pos):  
                    if total_loaded_cargo < max_cargo and credits_val >= p_map["Tech"]:
                        credits_val -= p_map["Tech"]; inventory["Tech"] += 1
                elif pygame.Rect(540, 406, 80, 26).collidepoint(mouse_pos):  
                    if inventory["Tech"] > 0:
                        credits_val += p_map["Tech"]; inventory["Tech"] -= 1
                elif pygame.Rect(250, 600, 400, 40).collidepoint(mouse_pos):
    ...
    if game_state == 'TRAVEL':
        ...
        if keys[pygame.K_LEFT]:
            player_angle -= 3.0 * dt; turn_dir = -1
            if random.random() < 0.4:
                r_pt = poly_transform([(12, -20)], WIDTH//2, HEIGHT//2, player_angle)[0]
                add_rcs_particle(r_pt[0], r_pt[1], math.sin(player_angle+1.5)*40, -math.cos(player_angle+1.5)*40)
        ...
        if keys[pygame.K_a] and fuel > 0:
            ...
            if random.random() < 0.4:
                r_pt = poly_transform([(15, 10)], WIDTH//2, HEIGHT//2, player_angle)[0]
                add_rcs_particle(r_pt[0], r_pt[1], sx2*50, sy2*50)
        ...
        if not keys[pygame.K_a] and not keys[pygame.K_d]:
            ...
            damp_amount = side_vel * (0.8 + rcs_level * 0.4) * dt
        ...
        if throttle > 0.1 and fuel > 0 and random.random() < 0.5:
            add_exhaust_particle(WIDTH//2 - fx*25, HEIGHT//2 - fy*25, -fx*80, -fy*80)

        for st in stations: st["rot"] = (st["rot"] + 0.5*dt) % 360.0

        # MEGA SCALE proximity detector (Increased to 380 units radius)
        active_station = None
        for st in stations:
            if math.hypot(world_x - st["x"], world_y - st["y"]) < 380:
                active_station = st
                current_radio_text = f"TOWER: Massive entity detected: {st['name']}. Press [E] to launch approach corridor."
    ...
    # ======================================================================
        ...
        if keys[pygame.K_w] and fuel > 0:
            dock_vy -= 220 * dt; fuel -= 0.8 * dt
            if random.random() < 0.4: add_rcs_particle(dock_ship_x, dock_ship_y+13, 0, 40)
        if keys[pygame.K_s] and fuel > 0:
            dock_vy += 160 * dt; fuel -= 0.8 * dt
            if random.random() < 0.4: add_rcs_particle(dock_ship_x, dock_ship_y-13, 0, -40)
        if keys[pygame.K_a] and fuel > 0:
            dock_vx -= 190 * dt; fuel -= 0.8 * dt
            if random.random() < 0.4: add_rcs_particle(dock_ship_x+22, dock_ship_y, 40, 0)
        if keys[pygame.K_d] and fuel > 0:
            dock_vx += 190 * dt; fuel -= 0.8 * dt
            if random.random() < 0.4: add_rcs_particle(dock_ship_x-22, dock_ship_y, -40, 0)
        ...
        for pid, pad in dock_pads.items():
            if pad["status"] == "OCCUPIED":
                ...
                if p_rect.colliderect(t_rect):
                    hull = max(0.0, hull - 20.0); current_radio_text = "TOWER: HANGAR IMPACT WARNING!"
        ...
        if assigned_pad:
            ...
            if abs(dock_ship_x - pad["x"]) < 20:
                ...
                if aligned:
                    if math.hypot(dock_vx, dock_vy) < 45: dock_landing_success = True; game_state = 'STATION_MENU'
    ...
    if game_state == 'MAIN_MENU':
        ...
        draw_ui_button(screen, pygame.Rect(280, 300, 320, 40), "SHIP UPGRADE SHOP", mouse_pos)
        draw_ui_button(screen, pygame.Rect(280, 350, 320, 40), "EXIT SIMULATOR", mouse_pos)
        ...
        draw_ui_button(screen, pygame.Rect(100, 280, 680, 35), f"Reinforce Hull Plating (Max: {int(max_hull)}%) -> Cost: 100 CR", mouse_pos)
        rcs_txt = f"Upgrade RCS Stabilizer (Grade: {rcs_level}/3) -> Cost: 120 CR" if rcs_level < 3 else "RCS Stabilizer Systems Fully Upgraded (MAX)"
        draw_ui_button(screen, pygame.Rect(100, 330, 680, 35), rcs_txt, mouse_pos)
        ...
        # Draw Mega Stations in World Coordinates
        for st in stations:
            sx3, sy3 = int(WIDTH//2 + (st["x"] - world_x)), int(HEIGHT//2 + (st["y"] - world_y))
            # Large boundaries bounds check to ensure massive sprites don't clip early
            if -500 <= sx3 <= WIDTH+500 and -500 <= sy3 <= HEIGHT+500:
                pygame.draw.circle(screen, MGRAY, (sx3, sy3), 360, 1) # Orbit ring indicator
        ...
        draw_ui_button(screen, pygame.Rect(650, 150, 110, 28), "REFUEL (20)", mouse_pos, active=(fuel>=max_fuel))
        draw_ui_button(screen, pygame.Rect(650, 182, 110, 28), "REPAIR (30)", mouse_pos, active=(hull>=max_hull))
        
        screen.blit(FONT_MD.render("LOCAL COMMODITY COMMERCE TERMINAL:", True, WHITE), (60, 240))
        pygame.draw.rect(screen, DGRAY, (60, 275, 780, 200))
        pygame.draw.rect(screen, WHITE, (60, 275, 780, 200), 1)
        
        screen.blit(FONT_SM.render("COMMODITY      MARKET PRICE    ORDER ACTIONS", True, LGRAY), (80, 290))
        pygame.draw.line(screen, GRAY, (80, 310), (820, 310), 1)
        
        commodities_meta = [("Heavy Metals", "Metals", 330), ("Hydro Food", "Food", 370), ("Cyber Tech", "Tech", 410)]
        ...
        screen.blit(FONT_MD.render(f"Cargo Bay Loading Ledger: {total_loaded_cargo} / {max_cargo} Tons Inventory Loaded", True, WHITE), (60, 500))
        screen.blit(FONT_SM.render(f"Current Cargo Hold Manifest -> Metals: {inventory['Metals']} | Food: {inventory['Food']} | Tech: {inventory['Tech']}", True, LGRAY), (60, 530))
        
        draw_ui_button(screen, pygame.Rect(250, 600, 400, 40), "UNDOCK CARGO VESSEL INTO SPACE", mouse_pos)
